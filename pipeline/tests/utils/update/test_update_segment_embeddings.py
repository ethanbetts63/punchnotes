...
def _make_set(name, slug, start_seconds, video_number):
    ...
    return Set.objects.create(video=video, comedian=comedian, start_seconds=float(start_seconds))
...
def test_ensure_beat_segments_creates_rows_for_new_beats():
    ...
    standup_set = _make_set("Comic One", "comic-one", 10, 100)
...
def test_ensure_beat_segments_skips_beats_that_already_have_segments():
    ...
    standup_set = _make_set("Comic One", "comic-one-2", 10, 101)
    beat = _make_beat(standup_set, 1, 1, 1, 1)
...
def test_unembedded_beat_segments_builds_and_returns_missing_keys():
    ...
    standup_set = _make_set("Comic One", "comic-one-3", 20, 102)
    ...
    assert result == [{"key": "ep102.ts20.bit003.beat004.seg001", "text": "A single punchline segment."}]
...
def test_unembedded_beat_segments_excludes_beats_without_joke_type():
    ...
    standup_set = _make_set("Comic One", "comic-one-4", 10, 103)
...
def test_ingest_segment_embeddings_stores_by_key():
    ...
    standup_set = _make_set("Comic One", "comic-one-6", 20, 105)
    ...
    result = ingest_segment_embeddings([
        {"key": "ep105.ts20.bit003.beat004.seg001", "embedding": [1.0, 2.0]},
    ])
...
def test_ingest_segment_embeddings_matches_by_truncated_start_seconds():
    ...
    standup_set = _make_set("Comic One", "comic-one-6b", 20.7, 107)
    beat = _make_beat(standup_set, 3, 4, 1, 1)
    segment = BeatSegment.objects.create(beat=beat, ordinal=1, text="text", line_start=1, line_end=1)

    result = ingest_segment_embeddings([
        {"key": "ep107.ts20.bit003.beat004.seg001", "embedding": [1.0, 2.0]},
    ])
...
def test_ingest_segment_embeddings_reports_not_found_and_invalid_key():
    ...
    result = ingest_segment_embeddings([
        {"key": "ep999.ts10.bit001.beat001.seg001", "embedding": [1.0]},
...
def test_ingest_segment_embeddings_uses_bulk_lookup(django_assert_num_queries):
    ...
    standup_set = _make_set("Comic One", "comic-one-7", 20, 106)
    ...
    with django_assert_num_queries(2):
        result = ingest_segment_embeddings([
            {"key": "ep106.ts20.bit003.beat004.seg001", "embedding": [1.0]},
            {"key": "ep106.ts20.bit003.beat004.seg002", "embedding": [2.0]},
        ])
