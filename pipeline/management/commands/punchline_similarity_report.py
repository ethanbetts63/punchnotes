...
OUTPUT_FILENAME = "punchline_similarity_report.json"
...
class Command(BaseCommand):
    help = "Report similarity between jokes using punchline text embeddings"
    ...
    def handle(self, *args, **options):
        ...
        beats = list(qs)
        self.stdout.write(f"Loaded {len(beats)} beats with punchline embeddings.")
        ...
        for label, group in groups.items():
            ...
            for a, b in combinations(group, 2):
                if a.bit.set.comedian_id == b.bit.set.comedian_id:
                    continue
                sim = round(cosine_sim(a.punchline_embedding, b.punchline_embedding), 4)
                if sim >= threshold:
