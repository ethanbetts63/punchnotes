import type { Comedian, SetListItem, Video } from "@/lib/serverApi";
...
export function setToTile(set: SetListItem): TileData {
  ...
  return {
    ...
    meta,
    badges: (() => {
      const jokeBookSize = getJokeBookSize(set.attributes);
      return jokeBookSize ? [jokeBookBadge[jokeBookSize]] : [];
    })(),
  };
}
